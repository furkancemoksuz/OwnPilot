            ...
            {!claw.workspaceId ? (
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <button onClick={() => { setViewingFile(null); setFileContent(null); }}
                    className="p-1 rounded hover:bg-bg-tertiary dark:hover:bg-dark-bg-tertiary">
                    <ArrowLeft className="w-4 h-4 text-text-muted" />
                  </button>
                  <FileText className="w-4 h-4 text-text-muted shrink-0" />
                  <span className="text-sm font-medium text-text-primary dark:text-dark-text-primary truncate">{viewingFile}</span>
                  <div className="flex-1" />
                  <a href={`/api/v1/file-workspaces/${claw.workspaceId}/file/${viewingFile}?download=true`}
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div className="flex-1" />
                  <button onClick={() => loadFiles(currentFilePath)} className="text-xs text-primary hover:underline">Refresh</button>
                  <a href={`/api/v1/file-workspaces/${claw.workspaceId}/download`}
                    className="text-xs text-primary hover:underline ml-2" title="Download ZIP">
                    Download All
                  </a>
                </div>

                {isLoadingFiles ? <LoadingSpinner message="Loading files..." /> : workspaceFiles.length === 0 ? (
                  <p className="text-sm text-text-muted dark:text-dark-text-muted py-4 text-center">Workspace is empty. The claw will create files as it works.</p>
                ) : (
                  <div className="space-y-1">
                    ...
                    {[...workspaceFiles].sort((a, b) => {
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (file.isDirectory) {
                            loadFiles(currentFilePath ? `${currentFilePath}/${file.name}` : file.name);
                          } else {
                            loadFileContent(currentFilePath ? `${currentFilePath}/${file.name}` : file.name);
                          }
                        }}
