...
export function ChannelsPage() {
  return (
    <DocsLayout>
      <Badge variant="blue" className="mb-3">Channels</Badge>
      <h1>Channels</h1>
      <p className="text-lg text-[var(--color-text-muted)] mb-8">
        OwnPilot connects to Telegram and WhatsApp via channel plugins, bringing the full assistant
        experience — 190+ tools, memory, workflows — directly into your messaging apps.
        Each channel has approval flow, pairing keys, user management, and inbox persistence.
      </p>
      ...
      <table>
        ...
        <tbody>
          <tr><td>Telegram</td><td>Production</td><td>Bot token</td><td>Full feature support</td></tr>
          <tr><td>WhatsApp</td><td>Production</td><td>QR code scan</td><td>Multi-device (Baileys)</td></tr>
        </tbody>
      </table>

      <Callout type="note" title="Channel plugin architecture">
        Channels are implemented as plugins using a builder pattern:
        {" "}<code>createChannelPlugin().meta().platform().channelApi(factory).build()</code>.
        The Universal Channel Protocol (UCP) provides a unified message format across channels.
      </Callout>
      ...
      <ol>
        ...
        <li>Copy the bot token (format: <code>1234567890:AABBccDDeeFF...</code>)</li>
        <li>Optionally: use <code>/setprivacy</code> to disable group privacy for group use</li>
      </ol>

      <h3>Step 2: Configure the token</h3>
      <p>You can configure via <code>.env</code>, the Config Center web UI, or the REST API:</p>
      ...
      <Callout type="tip" title="Telegram bot in Docker">
        When running in Docker, the Telegram bot starts automatically if <code>TELEGRAM_BOT_TOKEN</code> is set.
        You don't need to run <code>ownpilot bot start</code> manually.
      </Callout>
      ...
      <table>
        ...
        <tbody>
          <tr><td>Streaming responses</td><td>Messages delivered in chunks as the AI generates them</td></tr>
          <tr><td>Markdown formatting</td><td><code>*bold*</code>, <code>_italic_</code>, <code>`code`</code>, links</td></tr>
          <tr><td>File/image support</td><td>Photos, documents, and voice messages accepted</td></tr>
          <tr><td>Inline keyboards</td><td>Approval dialogs and action buttons sent as inline keyboards</td></tr>
          <tr><td>Group chats</td><td>Bot responds when mentioned or in private conversations</td></tr>
          <tr><td>User approval</td><td>Only approved users can interact with the bot</td></tr>
        </tbody>
      </table>
      ...
      <ol>
        <li>Open the web UI: <strong>Settings → Channels → WhatsApp → Connect</strong></li>
        <li>A QR code is displayed (30-second expiry)</li>
        <li>Open WhatsApp on your phone</li>
        <li>Go to <strong>Settings → Linked Devices → Link a Device</strong></li>
        <li>Scan the QR code</li>
        <li>Connection status updates to <code>connected</code> in real-time via WebSocket</li>
      </ol>

      <Callout type="warning" title="WhatsApp ToS">
        Using unofficial WhatsApp integrations may violate WhatsApp's Terms of Service.
      ...
      <p>
        Both Telegram and WhatsApp channels support a user approval system. When a new user
        messages the bot, they are added to a pending queue. An admin must approve them before
        they can interact with the assistant.
      </p>
      ...
      <table>
        ...
        <tbody>
          <tr><td><code>open</code></td><td>Anyone can use the bot (no approval required)</td></tr>
          <tr><td><code>allowlist</code></td><td>Only pre-approved users can interact</td></tr>
          <tr><td><code>request</code></td><td>Users request access; admin approves via web UI</td></tr>
        </tbody>
      </table>

      <h2>Pairing keys</h2>
      <p>
        Pairing keys are rotating short codes used to verify channel ownership and link
        a Telegram/WhatsApp user account to an OwnPilot user account.
      </p>
      ...
      <ol>
        <li><strong>Audit middleware</strong> — Tamper-evident log entry created</li>
        <li><strong>Persistence middleware</strong> — Message stored in conversation history</li>
        <li><strong>Post-processing</strong> — Message normalization and metadata extraction</li>
        <li><strong>Context injection</strong> — Memories, goals, and soul context prepended</li>
        <li><strong>Agent execution</strong> — Tool orchestration and LLM inference</li>
        <li><strong>Channel delivery</strong> — Response delivered back via the channel API</li>
      </ol>

      <Callout type="info" title="Shared conversation history">
        A conversation started on Telegram can continue in the web UI, and vice versa.
        All channels share the same conversation history for a given user.
      </Callout>
