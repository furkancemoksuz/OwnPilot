...
export function IntroductionPage() {
  return (
    <DocsLayout>
      <div className="mb-2">
        <Badge variant="blue" className="mb-3">Documentation</Badge>
      </div>
      <h1>Introduction</h1>
      <p className="text-xl text-[var(--color-text-muted)] leading-relaxed mb-6 mt-2 font-medium">
        OwnPilot is a privacy-first personal AI assistant platform that runs entirely on your own infrastructure.
        Self-hosted. Your data stays yours.
      </p>

      <Callout type="tip" title="Quick Start">
        Want to get running immediately? Jump to the{" "}
        <Link to="/docs/quick-start">Quick Start guide</Link> — Docker gets you live in under 2 minutes.
      </Callout>

      <h2>What is OwnPilot?</h2>
      <p>
        OwnPilot is a full-featured AI assistant platform that you deploy on your own hardware or cloud VM.
        It brings together multi-provider AI (96 providers including local inference), 190+ built-in tools,
        autonomous soul agents, visual workflow automation, MCP integration, voice, browser automation,
        IoT control, and Telegram + WhatsApp connectivity — all in a single, self-hosted package.
      </p>
      <p>
        Unlike cloud-based AI assistants, OwnPilot never sends your personal data to third parties.
        Your conversations, memories, notes, tasks, contacts, and all other personal data stay on your server.
      </p>
      ...
      <table>
        ...
        <tbody>
          <tr><td>AI Providers</td><td>96 (including 8 aggregators + local)</td></tr>
          <tr><td>Built-in Tools</td><td>190+ across 32 categories</td></tr>
          <tr><td>Workflow Node Types</td><td>23</td></tr>
          <tr><td>Test Suite</td><td>26,500+ tests, 545+ files</td></tr>
          <tr><td>Codebase</td><td>~180K LOC (TypeScript)</td></tr>
          <tr><td>License</td><td>MIT</td></tr>
          <tr><td>Runtime</td><td>Node.js 22+</td></tr>
          <tr><td>Database</td><td>PostgreSQL 16+ with pgvector</td></tr>
        </tbody>
      </table>

      <h2>Architecture overview</h2>
      <p>
        OwnPilot is a TypeScript monorepo managed with Turborepo and pnpm. It consists of 4 packages:
      </p>
      <ul>
        <li><code>@ownpilot/core</code> — AI engine, 190+ tools, plugins, sandbox, crypto, privacy (~62K LOC)</li>
        <li><code>@ownpilot/gateway</code> — Hono HTTP API server, 120+ route modules, 45+ DB repositories (~76K LOC)</li>
        <li><code>@ownpilot/ui</code> — React 19 + Vite 7 + Tailwind CSS 4 frontend, 57+ pages (~40K LOC)</li>
        <li><code>@ownpilot/cli</code> — Commander.js CLI for server and configuration management</li>
      </ul>
      <p>
        In production, a single port (8080) serves everything: the bundled React UI, REST API, WebSocket (<code>/ws</code>),
...
# Open http://localhost:8080 — configure your first AI provider in Settings → Config Center`}
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {[
          ...
          { href: "/docs/agents", title: "Agent System", desc: "Soul agents, background agents, crews" },
        ].map((link) => (
          <Link
            key={link.href}
            to={link.href}
